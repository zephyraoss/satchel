...
Satchel provides portable Docker volumes for small clusters. A volume uses a local ext4 filesystem while mounted and replicates changed blocks to an S3-compatible bucket. When a service moves to another node, Satchel restores the latest published generation before starting the container.
...
By default, `fsync` and FUA writes wait until Satchel publishes the current generation to S3. A database can therefore treat a successful `fsync` as durable across node loss. Volumes that favor latency over recovery point can set `durability=async`; those volumes may lose writes made since the last sync interval, which defaults to five seconds.
...
2. Compresses adjacent changed blocks into one segment.
3. Uploads the immutable segment and manifest.
4. Advances `state.json` with a conditional S3 write.
...
Satchel creates a full checkpoint after 128 generations at a safe filesystem flush or clean unmount. Current restores stop at that checkpoint, while its parent link keeps older generations available for point-in-time recovery. The default history window is seven days. Garbage collection marks older objects first, waits 24 hours, then deletes anything that is still outside the retained history.
...
| `durability` | `remote`, `async` | `remote` makes `fsync` wait for S3 and is the default. `async` acknowledges local durability and publishes on the sync interval. |
